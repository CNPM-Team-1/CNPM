...
public class ReceiptAddController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Add customer to customerHolder for auto complete
    ...
    @FXML
    void showChosenCustomer(KeyEvent event) {
        if (event.getCode().equals(KeyCode.ENTER)) {
            ...
            List<Orders> ordersList = OrdersRepository.getIncompleteLikeCustomerName(session, customerHolder.getText());
        ...
        if (event.getClickCount() == 2) {
            try {
                ...
                if (ordersDetailList != null && !ordersDetailList.isEmpty()) {
                    ...
                    Long sumAmount = ordersDetailModelList.stream().mapToLong(t -> Long.parseLong(NumberHelper.removeComma(t.getFinalAmount()))).sum();
